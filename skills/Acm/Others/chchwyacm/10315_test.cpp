...
TEST(PokerHandTest, compareTwoPair ){
    ...
    PokerHand p1,p2;
    p1.fetchCard("6C 6H 8C 8C TC"); //8兔胚
    p2.fetchCard("AC AH 2H 9D 9C"); //9兔胚
    //p2 > p1
    EXPECT_EQ( p1.compareTo(p2), -1 );
    EXPECT_EQ( p2.compareTo(p1), 1 );
...
TEST( PokerHand, comparePair ){
    ...
    PokerHand p1,p2;
    //不同胚
    p1.fetchCard("3C 6H 8C 8C TC"); //8胚
    p2.fetchCard("2C AH 8H AD 9C"); //A胚

    EXPECT_EQ( p2.compareTo(p1) , 1);

    //同胚比其他點數
    p1.fetchCard("3C 6H 8C 8C TC"); //8胚
    p2.fetchCard("2C AH 8H 8D 9C"); //8胚
    //2>1
    EXPECT_EQ( p1.compareTo(p2), -1);
    EXPECT_EQ( p2.compareTo(p1), 1);

    //只差最後一點數
    p1.fetchCard("3C 6H 8S 8C TC"); //8胚
    p2.fetchCard("2C 6H 8H 8D TC"); //8胚
    //1>2
    EXPECT_EQ( p1.compareTo(p2), 1);

    //完全平手
    p1.fetchCard("9C 5H 8C JC JS"); //j胚
    p2.fetchCard("9D 5H 8H JD JC"); //j胚
...
TEST(PokerHand, compareHighCard){
    PokerHand p1,p2;
    p1.fetchCard("3C 6H 8C KC TC"); //8胚
    p2.fetchCard("2C AH 8H 4D 9C"); //8胚
...
TEST(PokerHand, compareTie){
    ...
    PokerHand p1,p2;
    p1.fetchCard("7C 8H 9C TC JC"); //7順
    p2.fetchCard("7H 8D 9H TD JS");

    EXPECT_EQ( p1.compareTo(p2) , 0);

    p1.fetchCard("7C 7H 7H TC 7D"); //7鐵 出千了..
